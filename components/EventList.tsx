...
export default function EventList() {
    ...
    useEffect(() => {
        ...
        axiosUtil().get(`/tags`).then((response) => {
    ...
    function onFilter(filter: string) {
        axiosUtil().get(`/events?searchName=${filter}`).then((response) => {
    ...
    const getMoreEvents = (getMore = true) => {
        ...
        axiosUtil().get(`/events?page=${page.current}`,).then((response) => {
            if (getMore) {
    ...
    return (
        <FlatList data={events}
                  ...
                  ListHeaderComponentStyle={{position: "absolute",marginBottom:4, top: 0, left: 0, right: 0, zIndex: 1}}
